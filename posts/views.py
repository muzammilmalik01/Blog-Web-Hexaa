...
class ScheduledPostPremiumUserMixin:  # Implementing DRY.
    ...
    def get_queryset(self):
        ...
        * Working on Lists Views and Detail Views  *
        ...
        if (
            user.is_anonymous
        ):  # Checking for anonymous (Unauthenticated Read-Only) requets.
            return (
                super()
                .get_queryset()
                .filter(posted_at__lte=timezone.now(), is_premium_post=False)
                .order_by("-posted_at")
            )
        ...
        return (
            super()
            .get_queryset()
            .filter(posted_at__lte=timezone.now())
            .order_by("-posted_at")
        )
...
class PremiumPostMixin:
    def get_queryset(self):
        ...
        return (
            super()
            .get_queryset()
            .filter(posted_at__lte=timezone.now(), is_premium_post=True)
        )
...
class ListAllPostsAPI(
    ScheduledPostPremiumUserMixin, PaginationMixin, generics.ListAPIView
):
    """
...
class DetailPostAPI(
    ScheduledPostPremiumUserMixin, generics.RetrieveUpdateDestroyAPIView
):
    """
    ...
    * Scheduling enabled *
    ...
    def get_object(self):
        ...
        slug = self.kwargs.get(
            self.lookup_field
        )  # Get the value of slug from URL Params.
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        * How to Fix: Use Sessions to keep track of viewed Posts or Track the user's view count using IP Address. *
        ...
        next_post = (
            queryset.filter(
                Q(posted_at__lt=instance.posted_at)
                | (Q(posted_at=instance.posted_at) & Q(id__gt=instance.id))
            )
        ...
        previous_post = (
            queryset.filter(
                Q(posted_at__gt=instance.posted_at)
                | (Q(posted_at=instance.posted_at) & Q(id__lt=instance.id))
            )
            .order_by("posted_at", "-id")
            .first()
        )
        ...
        response["previous_post_slug"] = (
            previous_post.post_slug if previous_post else None
        )

        return Response(response)  # Returning the object.


class GetFeaturedPosts(
    ScheduledPostPremiumUserMixin, PaginationMixin, generics.ListAPIView
):
    """
...
class GetLikebyUserPost(generics.RetrieveAPIView):
    ...
    def get_object(self):
        ...
        obj = get_object_or_404(
            queryset, liked_by=self.kwargs["user_id"], post=self.kwargs["post_id"]
        )
        return obj
...
class DestroyPostLike(generics.DestroyAPIView):
    ...
    def get_object(self):
        ...
        obj = queryset.filter(
            post=post_obj, liked_by=user_obj
        ).first()  # Performing Look-up
...
class DestroyCommentLike(generics.DestroyAPIView):
    ...
    def get_object(self):
        ...
        obj = queryset.filter(
            comment=comment_obj, liked_by=user_obj
        ).first()  # Performing Look-up
    ...
    def create(self, request, *args, **kwargs):
        ...
        if customers:  # If the customer already exists in Stripe
            ...
            for subscription in customer.subscriptions.data:
                if (
                    subscription.plan.id == "price_1Oqs9jLbIWA3Cm6Ek5D2k46i"
            ...
            customer = stripe.Customer.create(
                email=request.user.email, source=data["source"]
            )
            subscription = stripe.Subscription.create(
                customer=customer.id,
                items=[{"price": "price_1Oqs9jLbIWA3Cm6Ek5D2k46i"}],
...
class DetailedNotification(generics.UpdateAPIView):
    """
    A view for updating detailed notifications.

    This view allows updating detailed notifications by using the `PUT` method.
    It uses the `NotificationsSerializier` serializer to serialize and validate the data.

    Attributes:
        queryset (QuerySet): The queryset containing all the notifications.
        serializer_class (Serializer): The serializer class for the notifications.
